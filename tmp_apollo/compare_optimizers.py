...
from optimizer_a import Apollo
...
def make_params(model, is_paddle, rank, update_proj_gap, apollo_scale, proj_type, proj, scale_type):
    ...
    if is_paddle:
        for module_name, module in model.named_sublayers():
            if not isinstance(module, paddle.nn.Linear):
                continue
            if not any(target_key in module_name for target_key in target_modules_list):
                continue
            print(f"Paddle Adding {module_name} to APOLLO parameters")
        ...
        for module_name, module in model.named_modules():
            ...
            if not any(target_key in module_name for target_key in target_modules_list):
                continue
            print(f"TorchAdding {module_name} to APOLLO parameters")
            lowrank_params.append(module.weight)
...
def train_model(config, steps=STEPS, use_apollo=True):
    losses_both = {}
    ...
    if use_apollo:
        param_groups = make_params(
            model_p,
            is_paddle=True,
            rank=config['dim'] // config['n_heads'],
            update_proj_gap=200,
            apollo_scale=1.0,
            proj_type='std',
            proj='random',
        ...
        optimizer_p = Apollo(
            ...
            scale_front=config['scale_front']
        )
    else:
    ...
    if use_apollo:
        print("Testing ApolloAdamW (PyTorch)...")
    else:
...
for config in configs:
    ...
    print('='*50)
    print('Test adamw apollo...')
    loss = train_model(config)

    results[config['name']] = {
        'ApolloAdamW': loss['torch'],
        'Apollo': loss['paddle']
    }

    # print('='*50)
    # print('Test original adamw...')
    # loss = train_model(config, use_apollo=False)
