...
class CLIPModel(CLIPPretrainedModel):
    ...
    def _get_name_mappings(cls, config: CLIPConfig) -> List[StateDictNameMapping]:
        ...
        model_type = config.get("model_type", "clip") if isinstance(config, dict) else "clip"
        ...
        if model_type in ["clip", "clip_text_model"]:
            num_text_layer = config.get(num_layer_key, config["text_config"][num_layer_key])

        if model_type in ["clip", "clip_vision_model"]:
            num_vision_layer = config.get(num_layer_key, config["vision_config"][num_layer_key])
